...
class Welcome extends Front_Controller
{
    ...
    public function appointment()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //     // Retrieve the patient details based on mobile number and email
            ...
            //         // Load a view and pass the result data
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user_password = $this->role->get_random_password($chars_min = 6, $chars_max = 10, $use_upper_case = false, $include_numbers = true, $include_special_chars = true);
            $user_name = $this->role->get_random_password($chars_min = 3, $chars_max = 3, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);
            $username = "pat" .$user_name .'_'. $insert_id;
    ...
    public function bookAppointment()
    {
        ...
        $global_shift = $this->onlineappointment_model->doctorGlobalShift();
        $this->data["global_shift"] = $global_shift;
        $params = $this->input->post("doctor") . ',' . 1 . ',' . $this->customlib->dateFormatToYYYYMMDD($this->input->post("date")) . ',' . $this->input->post("global_shift");
        $custom_fields = $this->customfield_model->getByBelongPatientPanel('appointment');
        foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
            if ($custom_fields_value['validation']) {
                $custom_fields_id = $custom_fields_value['id'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($this->input->post())) {
                ...
                $this->load_theme('pages/appointment', $this->config->item('front_layout'));
            ...
            try {
                ...
                // $this->db->where('patients.mobileno', $mobileno);
                $this->db->where('patients.email', $email);
            ...
            if ($patient_type == "new patient") {
                ...
                $this->session->set_flashdata("success_msg", $this->lang->line("success_message"));
                $this->mailsmsconf->mailsms('appointment_approved', $sender_details);
                $json_array = array('status' => '1', 'msg' => "Appointment Booked",'appointment_id'=> $insert_id);
                echo json_encode($json_array);
            } else {
                if (empty($this->session->userdata("patient"))) {
