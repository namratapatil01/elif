...
class Site extends Public_Controller
{
    ...
    public function getSlotByShift()
    {
        ...
        $class = "";
        // below code has been commented for adding the new condition of time to check with current time
        ...
        //     } else {
        //         array_push($data["result"], array("time" => $this->customlib->getHospitalTime_FormatFrontCMS($time), "class" => "bg-avail", "filled" => ''));
        //     }
        // }
        foreach ($array_of_time as $time) {
            ...
            // Set the time to the DateTime object
            $dateTime->setTime(date('H', strtotime($formattedTime)), date('i', strtotime($formattedTime)), date('s', strtotime($formattedTime)));
            ...
            if (!empty($appointments)) {
                foreach ($appointments as $appointment) {
                    if ($appointment->time == $formattedTime) {
                        $class  = "bg-not-avail";
                        $filled = "filled";
                        break;
                    }
            ...
            array_push($data["result"], array(
                "time" => $this->customlib->getHospitalTime_FormatFrontCMS($time),
                "class" => $class,
                "filled" => $filled,
